...
Store `$WT` for the entire loop. From this point forward, every git operation that touches the working branch uses `git -C "$WT"`; every file read/write/edit inside the working tree uses `$WT/`-prefixed paths; every test/lint command runs via `cd "$WT" &&`. The main working tree (your cwd) is read-only from here on.
...
Pull the latest working branch first — the remote may have moved since Step 1:

```bash
git -C "$WT" checkout {working-branch} && git -C "$WT" pull
```

For each issue-handler that committed its branch:

1. `git -C "$WT" checkout {working-branch}` (already on it from the pull above)
